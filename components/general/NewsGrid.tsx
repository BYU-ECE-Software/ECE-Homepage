...
export default function NewsGrid({ items, featuredFirst = false, columns = 3 }: NewsGridProps) {
  if (items.length === 0) {
    return <p className="rounded-lg bg-slate-50 p-8 text-center text-slate-600">No stories are currently posted.</p>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`grid gap-6 ${gridClass}`}>
        {(featuredFirst ? rest : items).map((item) => <NewsCard key={`${item.title}-${item.date}`} item={item} />)}
      </div>
    </div>
  );
}
