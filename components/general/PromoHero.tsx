...
export default function PromoHero({
  ...
  return (
    <section className={`relative isolate overflow-hidden bg-linear-to-br from-byu-navy via-[#003f7d] to-byu-royal px-6 py-20 text-white sm:py-28 ${centered ? "text-center" : ""}`}>
      {image && (
        <>
          <Image src={image} alt={imageAlt} fill priority className="-z-20 object-cover" sizes="100vw" />
          <div className="absolute inset-0 -z-10 bg-byu-navy/75" />
        </>
      )}
          ...
          <div className={`mt-8 flex flex-wrap gap-3 ${centered ? "justify-center" : ""}`}>
            {actions.map((action) => {
              ...
              return action.href.startsWith("/") ? (
                <Link key={action.label} href={action.href} className={classes}>{action.label}</Link>
              ) : (
                <a key={action.label} href={action.href} target="_blank" rel="noopener noreferrer" className={classes}>{action.label}</a>
              );
            })}
          </div>
        )}
